...
const placeFirstRow = (s, square, color) => {
	...
	for(let i = 0;i < 8;i++){
		square[i].innerHTML = `<img class="nc" data-color="${color}"data-moveCount= "0" src="images/${position[i]}">`;
	}
}

const placePawns = (s, square, color) =>{
	for(let i = 0;i < 8;i++){
		square[i].innerHTML = `<img class="nc"	data-color="${color}" data-moveCount= "0" src="images/${color}p.png">`;
